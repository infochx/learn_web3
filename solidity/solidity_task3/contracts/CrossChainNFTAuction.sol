...
contract CrossChainNFTAuction is ReentrancyGuard, Ownable {
    ...
    function processCrossChainBid(
        ...
        // if (oldWinner != address(0) && oldBid > 0) {
        //     // 构造退款消息
        //     Client.Any2EVMMessage memory refundMessage = Client.Any2EVMMessage({
        //         destinationSelector: _selector(MessageProcessor.processCrossChainRefund),
        //         sourceChainSelector: Client.ChainSelector(auction.sourceChainId),
        //         data: abi.encode(oldWinner, oldBid)
        //     });

        //     // 发送退款消息
        //     ccipRouter.send{value: 0}(auction.sourceChainId, refundMessage);
        // }
    ...
    function endAuction(bytes32 auctionId) external nonReentrant {
        ...
        if (auction.currentWinner != address(0) && auction.currentBid > 0) {
            ...
            if (auction.paymentToken == address(0)) {
                // 以太币支付
                (bool success, ) = auction.nftOwner.call{
                    value: auction.currentBid
                }("");
                require(success, "Payment failed");
            } else {
                // ERC20代币支付
                // require(IERC20(tokenAddress).transfer(auction.nftOwner, auction.currentBid), "Transfer failed");
            ...
            if (auction.sourceChainId == block.chainid) {
                ...
                CrossChainNFTBridge(bridgeContract).unlockNFT(
                    auction.nftLockId,
                    auction.currentWinner
                );
            }
    ...
    function cancelAuction(bytes32 auctionId) external nonReentrant {
        ...
        if (auction.sourceChainId == block.chainid) {
            ...
            if (auction.paymentToken == address(0)) {
                // 以太币支付
                (bool success, ) = auction.currentWinner.call{
                    value: auction.currentBid
                }("");
                require(success, "Payment failed");
            } else {
